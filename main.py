...
def load_config():
    '''Загрузка конфига происходит только первй раз при запуске демона, поэтому нет необходимости логировать ошибки'''
...
def convert_callback(update, context):
    if len(context.args) != 4:
        ...
        try:
            ...
            update.message.reply_text(currencies_info['rates'][cur_to] * float(count_from))
        except ValueError:
            update.message.reply_text(f"The firts param should be a number, not the: `{count_from}`")
        except Exception as e:
            update.message.reply_text(e)

        print(count_from, cur_from, cur_to)
...
def main():
    ...
    currencies_url = config["urls"]["currencies_api"]
